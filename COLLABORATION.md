...
4. **Config-driven UI copy** — welcome text and page titles come from `APP_WELCOME_TEXT` / `APP_HOME_TITLE` so the same tests can target different apps without code changes.

5. **Developer ergonomics** — folder/platform test runner, headed scripts for manual OTP login, and English docs after an initial Polish pass.
...
2. Set `APP_WELCOME_TEXT` and `APP_HOME_TITLE` to match your app’s UI.
